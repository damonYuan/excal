...
import com.damonyuan.tcal.common.DefaultObservable;
import com.damonyuan.tcal.common.Observable;
import com.damonyuan.tcal.common.Observer;
...
//    	this(25569+(date.getTime()/86400000L));
    ...
    private void checkSerialNumber() {
        if ((serialNumber < minimumSerialNumber()) || (serialNumber > maximumSerialNumber())) {
            throw new TcalException("Date's serial number is outside allowed range");
        }
    }
    ...
    private long advance(final Date date, final int n, final TimeUnit units) {
        switch (units) {
            ...
            case Months: {
                ...
                if (y <= 1900 || y > 2199) {
                    throw new TcalException("year out of bounds. It must be in [1901,2199]");
                }
            ...
            default:
                throw new TcalException("undefined time units"); // TODO: message
        }
    }
    ...
    public static final Date nthWeekday(final int nth, final Weekday dayOfWeek, final int month, final int year) {
        if (nth <= 0) {
            throw new TcalException("zeroth day of week in a given (month, year) is undefined");
        }
        if (nth >= 6) {
            throw new TcalException("no more than 5 weekday in a given (month, year)");
        }
    ...
    public static Date min(final Date... t) {
        if (t == null) {
            throw new TcalException("argument cannot be null");
        }
        if (t.length == 0)
            return new Date();
        else {
    ...
    private static final long fromDMY(final int d, final int m, final int y) {
        if (y <= 1900 || y > 2199) {
            throw new TcalException("year(" + y + ") out of bound. It must be in [1901,2199]");
        }
        if (m <= 0 || m >= 13) {
            throw new TcalException("month outside JANUARY-December range [1,12]");
        }
        final boolean leap = isLeap(y);
        final int len = monthLength(m, leap);
        final int offset = monthOffset(m, leap);
        if (d <= 0 || d > len) {
            throw new TcalException("day outside month day-range");
        }
