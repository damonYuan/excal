...
public class Period implements Cloneable {
    ...
    public Period(final Frequency f) {
        switch (f) {
            ...
            case Daily:
                ...
                break;
            default:
                throw new TcalException(UNKNOWN_FREQUENCY); // QA:[RG]::verified
        }
    }
            ...
            switch (this.units) {
                case Years:
                    switch (another.units()) {
                        ...
                        default:
                            throw new TcalException(UNKNOWN_TIME_UNIT);
                    }
                    break;
                case Months:
                    switch (another.units()) {
    ...
    public Period divAssign(final int scalar) {
        ...
        if (this.length % scalar == 0)
            ...
            throw new TcalException("cannot be divided by " + scalar);
    ...
    public boolean lt(final Period another) {
        ...
        if (period1MaxDays < period2MinDays)
            ...
            throw new TcalException(UNDECIDABLE_COMPARISON);
    }
        ...
        switch (units) {
            ...
            case Days:
                if (length==1)
                    ...
                    return Frequency.OtherFrequency;
            default:
                throw new TcalException(UNKNOWN_TIME_UNIT); // QA:[RG]::verified
        }
    }
